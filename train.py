...
cov_1.append(np.array([[1,0],[0,1]]))
cov_2.append(np.array([[1,0],[0,1]]))

# Calculate propability of each point belonging to group 1,2
prob_1 = multivariate_normal.pdf(data, mean_1[-1], cov_1[-1])
prob_2 = multivariate_normal.pdf(data, mean_2[-1], cov_2[-1])
...
while len(group) < 2 or np.any(group[-1] != group[-2]):
    ...
    cov_1.append(np.cov(data[group[-1]==group_1], rowvar=0))
    cov_2.append(np.cov(data[group[-1]==group_2], rowvar=0))
...
plot_gif.gif(data, group, mean_1, mean_2, cov_1, cov_2)
